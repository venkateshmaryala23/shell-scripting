...
CREATE () {
  ...
  if [ $COUNT -eq 0 ]; then
     ...
     for x in $ec2_state_code;do
        ...
        if [ $x == 0 ]; then
          #"\e[1;32mSUCCESS\e[0m"
          echo "\e[1;32m$1 Instance is exists but it is in pending status\e[0m"
        elif [ $x == 16 ]; then
          echo  -e "\e[1;32m$1 Instance is already exists, It is in running state \e[0m"
        elif [ $x ==  32 ]; then
          echo "\e[1;32m$1 Instance is already exist but its statu is shutting-down \e[0m"
        elif [ $x == 64 ]; then
